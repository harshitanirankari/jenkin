pipeline {
  ...
  stages {
    ...
    stage('Load Environment Variables') {
      steps {
        script {
            ...
            envVars.each { line ->
              if (line.trim() && !line.startsWith('#')) {
                ...
                if (parts.size() == 2) {
                  ...
                  // Use this approach instead of env.setProperty
                  env."${key}" = value
                  echo "Set ${key}=${value}"
                }
              }
          ...
          echo "Loaded IMAGE_NAME=${env.IMAGE_NAME}"
          echo "Loaded CONTAINER_PORT=${env.CONTAINER_PORT}"
          echo "Loaded DEPLOY_PORT=${env.DEPLOY_PORT}"
        }
      }
    }
    ...
    stage('Build') {
      steps {
        script {
          // Extra validation to debug the issue
          echo "Building with IMAGE_NAME=${env.IMAGE_NAME}"
